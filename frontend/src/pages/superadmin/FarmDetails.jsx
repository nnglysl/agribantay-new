...
export default function SuperAdminFarmDetails() {
  ...
  const disposalYears = useMemo(() => {
    ...
    list.forEach(r => {
      if (r.disposal_date_raw) years.add(new Date(r.disposal_date_raw).getFullYear())
    })
  ...
  const serviceRequestYears = useMemo(() => {
    const list = serviceRequestData?.requests || []
    const years = new Set([new Date().getFullYear()])
    list.forEach(r => {
      if (r.request_date_raw) years.add(new Date(r.request_date_raw).getFullYear())
    })
    return [...years].sort((a, b) => b - a)
  }, [serviceRequestData])
